...
class Config:
    ...
    window_size = 10 # The size of the window to use.
    ### YOUR CODE HERE
...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    for sentence, labels in data:
        ...
        for center in range(len(sentence)):
            features = sentence[center]
            for i in range(1, window_size+1):
                left = center - i
                right = center + i
                left_feature = sentence[left] if left >= 0 else start
                right_feature = sentence[right] if right < len(sentence) else end
                features = left_feature + features + right_feature
            windowed_data.append((features, labels[center]))
		### END YOUR CODE
...
class WindowModel(NERModel):
    ...
    def add_prediction_op(self):
        ...
        b1 = tf.get_variable("b1", shape=[self.config.hidden_size], initializer=xavier(seed=1))
        b2 = tf.get_variable("b2", shape=[self.config.n_classes], initializer=xavier(seed=2))
        W = tf.get_variable("W", shape=[self.config.n_window_features * self.config.embed_size, self.config.hidden_size], initializer=xavier(seed=3))
        U = tf.get_variable("U", shape=[self.config.hidden_size, self.config.n_classes], initializer=xavier(seed=4))
