...
class GRUCell(tf.nn.rnn_cell.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope):
            ...
            #initFunc = tf.contrib.layers.xavier_initializer(uniform=False) # When gradient clipping is not needed, if uniform is True, then even without gradient clipping, GRU performs well in q3_gru.py.
            initFunc = tf.contrib.layers.xavier_initializer(uniform=True)
            W_r = tf.get_variable('W_r', [self.input_size, self.state_size], initializer=initFunc, dtype = tf.float32)
            U_r = tf.get_variable('U_r', [self.state_size, self.state_size], initializer=initFunc, dtype = tf.float32)
            b_r = tf.get_variable('b_r', [self.state_size,], initializer=tf.constant_initializer(0), dtype = tf.float32)
            W_z = tf.get_variable('W_z', [self.input_size, self.state_size], initializer=initFunc, dtype = tf.float32)
            U_z = tf.get_variable('U_z', [self.state_size, self.state_size], initializer=initFunc, dtype = tf.float32)
            b_z = tf.get_variable('b_z', [self.state_size,], initializer=tf.constant_initializer(0), dtype = tf.float32)    ## Recommend on Piazza
            W_o = tf.get_variable('W_o', [self.input_size, self.state_size], initializer=initFunc, dtype = tf.float32)
            U_o = tf.get_variable('U_o', [self.state_size, self.state_size], initializer=initFunc, dtype = tf.float32)
