...
class RNNModel(NERModel):
    ...
    def add_placeholders(self):
        ...
        """
        ### YOUR CODE HERE (~4-6 lines)
        self.input_placeholder = tf.placeholder(tf.int32, (None, self.max_length, Config.n_features), 'input')
    ...
    def add_prediction_op(self):
        ...
        with tf.variable_scope("RNN"):
            for time_step in range(self.max_length):
                ...
                dropout = tf.nn.dropout(state, dropout_rate)
                output = tf.matmul(dropout, U) + b2
                preds.append(output)
                ### END YOUR CODE
